...
- A Debug product may exist at the canonical `/private/tmp/AnyUprightDerivedData/Build/Products/Debug/AnyUpright.app` path (also spelled `/tmp/...`), Xcode's default `~/Library/Developer/Xcode/DerivedData/AnyUpright-*/Build/Products/Debug/AnyUpright.app`, a caller-supplied `<derived-data-path>/Build/Products/Debug/AnyUpright.app`, or a manually copied location. Use only the canonical path for host validation and treat every other registered Debug path as a possible stale-registration conflict.
