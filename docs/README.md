...
- FxAnalysis only accepts a `CMTimeRange`; it has no exact-one-frame request. The shared 0.05-second range is a probe window, not a frame-count guarantee. Upright and Horizon atomically claim the first `analyzeFrame` callback and return successfully from later callbacks, so one button click produces one model inference and one lifecycle analysis event while normal `cleanupAnalysis` still runs.
