...
- ScaleLSD has an offline fixed-512 Swift/Core ML migration path for quality validation: `tools/build-scalelsd-coreml.py` converts the official neural forward, `AnyUprightScaleLSDPostprocessor.swift` reproduces official junction/HAFM/wireframe decoding, and `tools/validate-holicity-upright.py --detector scalelsd-coreml` runs the shared HoliCity metric. `AnyUprightUprightProposal.swift` and `tools/rank-scalelsd-upright-proposals.swift` add an offline detector-independent VP/pair/transform/rejection stage, optionally anchored by camera priors from `tools/export-geocalib-camera-priors.swift`. The corrected sample100 raw availability remains strong (`96/99/95` for Vertical/Horizontal/Full), but selected Full proposals are only `7/100` correct and false accepts remain too high. This path is not selected or packaged by the production Upright plug-in and must not write parameters.
