...
- Inner Stretch and Outer Stretch both expose `Set Corner Keyframe`. One click adds a keyframe at the host's current timeline time to each of the eight existing corner X/Y slider channels. Channels that already have a keyframe at that time are left unchanged, and the host groups newly added keyframes into one undo operation when its undo API is available.
