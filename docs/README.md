...
- FxAnalysis only accepts a `CMTimeRange`; it has no exact-one-frame request. Horizon and Upright now request two input sampling intervals while retaining the verified 0.05-second minimum. They process only the first callback with a readable IOSurface and non-empty pixel bounds; if all image preparation paths fail, the claim is released so a later callback in the probe can take over. A rejected Horizon estimate and a valid Upright zero-candidate result are completed analyses and do not run again. Zero callbacks or unusable frames preserve existing parameters and candidates, while a valid Upright zero-candidate result intentionally clears old candidates.
...
xcrun swiftc AnyUpright/Plugin/AnyUprightCoreMLSessionLifecycle.swift AnyUprightTests/AnyUprightCoreMLSessionLifecycleTests.swift -o /tmp/AnyUprightCoreMLSessionLifecycleTests && /tmp/AnyUprightCoreMLSessionLifecycleTests
...
xcrun swiftc AnyUpright/Plugin/AnyUprightCoreMLSessionLifecycle.swift AnyUpright/Plugin/AnyUprightGeoCalibNeuralOutput.swift AnyUpright/Plugin/AnyUprightGeoCalibCoreML.swift AnyUprightTests/AnyUprightGeoCalibCoreMLCacheTests.swift -o /tmp/AnyUprightGeoCalibCoreMLCacheTests && /tmp/AnyUprightGeoCalibCoreMLCacheTests /Users/ibobby/Projects/AnyUpright
xcrun swiftc -O AnyUpright/Plugin/AnyUprightGeometry.swift AnyUpright/Plugin/AnyUprightLineDetection.swift AnyUpright/Plugin/AnyUprightCoreMLSessionLifecycle.swift AnyUpright/Plugin/AnyUprightGeoCalibNeuralOutput.swift AnyUpright/Plugin/AnyUprightGeoCalibCoreML.swift AnyUpright/Plugin/AnyUprightGeoCalibOptimizer.swift AnyUpright/Plugin/AnyUprightGeoCalibPreprocessGeometry.swift AnyUpright/Plugin/AnyUprightGeoCalibHorizonDetector.swift tools/evaluate-swift-geocalib-rotation.swift -o /tmp/AnyUprightSwiftGeoCalibFullValidationCompileCheck
