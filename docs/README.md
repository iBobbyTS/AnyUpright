...
- `Analyze Horizon` starts FxAnalysis near the current parameter time when the host provides one, runs the project-owned GeoCalib Swift/Metal roll detector, and writes only the accepted centered rotation correction back to `Rotation`.
...
- If GeoCalib resources, Metal, or model execution are unavailable, the implementation falls back to the previous Vision horizon detection path and then to the shared Sobel/Hough horizontal-line detector.
...
- Current Horizon implementation uses GeoCalib primary roll estimation gated by `roll_uncertainty <= 3 deg` plus rejection when two or more lightweight verifiers disagree by more than 10 degrees. The Swift/Metal migration was verified in slices in `/Users/ibobby/Temp/AnyUprightAlgorithmWorkDirectory` before project integration. Project-owned pieces now include GeoCalib preprocessing/gate/verifier glue in `AnyUprightGeoCalibHorizonDetector.swift`, the LM optimizer in `AnyUprightGeoCalibOptimizer.swift`, the neural-forward runtime in `AnyUprightGeoCalibNeuralPrototype.swift` and `AnyUprightGeoCalib.metal`, and the flat runtime weight bundle under `AnyUpright/Plugin/GeoCalibRuntime/`. Motion and Final Cut Pro 12.2 have both been verified to run `Analyze Horizon`, accept GeoCalib results, and write only `Rotation`.
