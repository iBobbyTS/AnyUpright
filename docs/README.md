...
- Automatic scoring quality. ScaleLSD ranks axis-filtered lines by normalized wireframe support score. The Sobel/Hough fallback uses a simpler angle-plus-length compatibility score.
...
The current traditional line detector is a CPU reference implementation based on Sobel edges, gradient-constrained Hough voting, and simple non-maximum suppression. It is used as the fallback for Upright candidate detection when ScaleLSD cannot run. Exact transform parameters are still solved by the shared geometry layer from the selected reference lines.
