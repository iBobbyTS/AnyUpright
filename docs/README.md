...
- Manual Vertical supports zero, one, or two contributing vertical guide lines: zero skips correction, one applies rotation only, and two solve the existing direct output-to-source matrix from those source-image guide lines so the referenced lines become vertical after correction. Older non-direct vertical perspective paths use a centered keystone transform around the horizontal centerline. Positive values move the top inward and bottom outward; negative values move the top outward and bottom inward.
- Manual Horizontal supports zero, one, or two contributing horizontal guide lines: zero skips correction, one applies rotation only, and two use the symmetric existing direct output-to-source matrix so the referenced source-image guide lines become horizontal after correction. Older non-direct horizontal perspective paths use a centered keystone transform around the vertical centerline. Positive values move the right side inward and left side outward; negative values move the right side outward and left side inward.
- Manual Full uses one unified guided-matrix entry point for zero to four contributing guide lines. Zero skips correction; one applies rotation only; one vertical plus one horizontal uses affine rectification to make the two guides perpendicular and axis-aligned; two same-orientation guides preserve the verified Vertical or Horizontal direct matrix; three guides preserve the same-orientation direct perspective matrix and append the minimum axis-preserving shear needed to align the remaining guide; two vertical plus two horizontal guides preserve the verified Full vanishing-point matrix.
...
- Rotation is stored internally for older correction paths and applies only in `Full` direction. Manual Vertical and Manual Horizontal direct-matrix rendering do not consume rotation.
...
- Manual vertical/horizontal modes support 0, 1, or 2 contributing lines. Manual full mode supports 0, 1, 2, 3, or 4 contributing lines. Zero contributing lines means no manual upright correction. Semi-auto and full-auto still use detected candidate slots and write the older stored correction values unless their selected references are rendered through the manual direct-matrix path.
