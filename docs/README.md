...
- In `AnyUpright Outer Stretch`, the four offsets are a destination/output corner selection and map back to the full source frame, matching the direction a user sees in the Motion canvas.
...
- `Mode` chooses only how reference lines are selected: `Manual` uses enabled manual guides, `Semi Auto` uses user-selected detected candidates, and `Full Auto` uses an automatic selector. `Analyze` is a no-op in manual mode and starts candidate-line analysis in semi/full-auto modes.
...
- Manual, Semi Auto, and Full Auto all pass their selected reference lines into the same guided matrix solver and the same matrix application path. Control mode does not participate in transform solving or rendering.
...
- Full-auto mode analyzes matching candidate lines and displays up to two automatically selected lines per included orientation while `Edit Mode` is on. Full direction uses the V2 global scene-family selector; single-direction modes retain score-only selection. The selected lines enter the same matrix solver used by Manual and Semi Auto.
- Candidate detection uses the local ScaleLSD Core ML model and Swift decode path. Full-direction Semi Auto and Full Auto run the V2 VP/homography candidate ranker described below. Single-direction modes retain the score-only selector.
...
- Full auto: detect candidate lines and choose up to two highest-scoring references per included orientation automatically.
...
- Automatic scoring quality. Single-direction Semi Auto and Full Auto rank axis-filtered lines by normalized ScaleLSD wireframe support score. Full-direction Semi Auto and Full Auto use the frozen V2 pair ranker and risk model.
...
The current traditional line detector is a CPU reference implementation based on Sobel edges, gradient-constrained Hough voting, and simple non-maximum suppression. It remains available to reference tests and non-model workflows, but Semi Auto and Full Auto do not use it as a ScaleLSD fallback. Exact transform parameters are still solved by the shared geometry layer from the selected reference lines.
...
- ScaleLSD is the primary production Upright candidate detector for Semi Auto and Full Auto. `AnyUprightScaleLSDDetector.swift` uses the shared Apple Silicon Metal resampler to produce its normalized fixed-512 grayscale input, `AnyUprightScaleLSDCoreML.swift` owns a model-specific facade over an independent `.fixed512` lifecycle cache, and `AnyUprightScaleLSDPostprocessor.swift` reproduces official junction/HAFM/wireframe decoding. Plugin registration asynchronously prewarms the fixed model, and Analyze uses the same configurable 15/30/60-second lifecycle policy as GeoCalib without sharing sessions or expiry state. The postprocessor uses a threshold-bounded uniform spatial grid for nearest-junction lookup; its retained linear implementation is a deterministic test oracle, not the production path. The ignored local FP16 model resource must exist at `AnyUpright/Plugin/ScaleLSDCoreML/scalelsd_neural_forward.mlmodelc`; Xcode packages it in the XPC service. If ScaleLSD cannot complete, later analysis callbacks may retry and cleanup preserves existing candidates; auto modes do not fall back to CPU Hough.
- Full-direction Semi Auto and Full Auto route the raw fixed-512 lines through `AnyUprightUprightV2CandidateGenerator.swift`, which reproduces the frozen V2 line deduplication, VP clustering, GeoCalib soft prior, discrete homography generation, crop/Jacobian scoring, and supporter features. `AnyUprightUprightV2TreeModel.swift` evaluates the bundled LightGBM pair and risk models under `Plugin/UprightV2`. Full Auto retains the risk gate and conservatively produces no selected guides when it fails; Semi Auto bypasses that gate so the user can inspect the best-ranked family. An unavailable GeoCalib prior falls back to the V2 no-prior branch; an unavailable V2 model or selector failure falls back to the prior score-only behavior.
- The selected V2 VP family is reduced with a deterministic quality-and-spatial-coverage selector. Full Auto receives at most two representative lines per orientation; Semi Auto receives at most ten per orientation, initially unselected. Both then use the same guided matrix solver as Manual. Single-direction Semi Auto keeps the top ten score-ranked candidates, while single-direction Full Auto keeps the top two. Switching from Semi Auto to Full Auto recomputes both displayed selection and runtime correction regardless of Edit Mode, so persisted manual `selected` flags and Semi Auto correction parameters cannot leak into Full Auto. The older offline proposal implementation in `AnyUprightUprightProposal.swift` is not part of production writeback.
...
- Upright Semi Auto and Full Auto OSC candidates are line-only controls: they do not draw endpoint handles and cannot be dragged. Every candidate uses the same orientation-specific color and 3-point line thickness as Manual guides; selected candidates additionally extend a same-color dashed direction guide to the canvas boundaries. Semi Auto still supports clicking the solid candidate line to toggle selection, while Full Auto remains read-only.
...
- In `Semi Auto` mode, click `Analyze`; only analyzed candidate lines matching `Direction` should be drawn and hittable. Full direction should show up to ten spatially distributed representatives per orientation from the best V2 family even when its risk score exceeds the Full Auto gate. Clicking candidates should toggle correction selection without allowing more than two selected lines per orientation.
- In `Full Auto` mode, click `Analyze`; Full direction should use the V2 selector and display up to two representative lines per orientation, while Vertical and Horizontal directions should retain score-only selection.
