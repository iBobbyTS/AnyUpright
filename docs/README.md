...
- In `AnyUpright Outer Stretch`, the four offsets are a destination/output corner selection and map back to the full source frame, matching the direction a user sees in the Motion canvas. The zero-offset OSC corners are converted from the source object's OBJECT frame into host CANVAS coordinates and rendered through the canvas-pixel overlay path, so they align with the visible image corners rather than the surrounding viewer canvas. After coordinate conversion, Inner and Outer share the same active-part resolution, circular handles, and yellow edge/face highlight renderer. The effect render uses a dedicated output-space quadrilateral coverage mask before projective sampling. The mask applies parameters in the stable correction frame, scales the result into the current render frame, and then crosses the physical-output Y boundary, preventing a strong-perspective homography horizon from exposing a clamped source edge while keeping the mask aligned with the OSC and homography target.
