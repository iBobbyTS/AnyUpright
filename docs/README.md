...
- Click `Detect Edge and Corner`; `Edit Mode` should be enabled, the manual source quad should not move, and detected independent edges/corners above `Score Threshold` should appear as green lines and green crosses.
- In Final Cut Pro, verify the `Quad` effect inspector shows `Edit Mode`, `Score Threshold`, and a `Detect Edge and Corner` button after restarting FCP. If only `Edit Mode` appears, the Motion template is missing the published custom-parameter and threshold targets.
