...
- FxAnalysis only accepts a `CMTimeRange`; it has no exact-one-frame request. The shared 0.05-second range is a probe window, not a frame-count guarantee. Upright atomically claims the first `analyzeFrame` callback and returns successfully from later callbacks, so variable frame rates cannot cause duplicate ScaleLSD inference while normal `cleanupAnalysis` still runs.
