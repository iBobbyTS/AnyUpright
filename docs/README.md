...
- In `AnyUpright Outer Stretch`, the four offsets are a destination/output corner selection and map back to the full source frame, matching the direction a user sees in the Motion canvas. The zero-offset OSC corners are converted from the source object's OBJECT frame into host CANVAS coordinates and rendered through the canvas-pixel overlay path, so they align with the visible image corners rather than the surrounding viewer canvas.
