...
- Plugin defaults: Horizon, Inner Stretch, and Upright share one AppKit/FxRemoteWindow wrapper, but each opens its own editor and persists a separate strongly typed plist. The files are `~/Library/Application Support/AnyUpright/Defaults/Horizon.plist`, `InnerStretch.plist`, and `Upright.plist`. Horizon stores `Fill Frame`; Inner Stretch stores `Ratio` and whether keyframe action notifications are suppressed; Upright stores `Direction`, `Mode`, and `Auto Crop`. Each window session snapshots factory defaults and the last saved settings in memory. Restore only changes the current form to factory values, while Save is the sole operation that writes the current form to its plist; each button is enabled only when its corresponding comparison differs. Saved parameter defaults are read while registering a new filter instance. The Inner Stretch notification preference is UI-only and is reloaded from its plist for every Set/Unset action, so it applies immediately to existing instances without entering render state. Other existing-instance behavior remains owned and persisted by FxPlug parameters. Outer Stretch has no defaults editor or plist.
