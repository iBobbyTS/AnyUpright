...
- Plugin defaults: Horizon, Inner Stretch, and Upright share one AppKit/FxRemoteWindow wrapper, but each opens its own editor and persists a separate strongly typed plist. The files are `~/Library/Application Support/AnyUpright/Defaults/Horizon.plist`, `InnerStretch.plist`, and `Upright.plist`. Horizon stores `Fill Frame`; Inner Stretch stores `Ratio`; Upright stores `Direction`, `Mode`, and `Auto Crop`. Saved values are read only while registering parameters for a new filter instance. Existing instances remain fully owned and persisted by their FxPlug parameters, and render, OSC, and analysis paths never read these plist files. Outer Stretch has no defaults editor or plist.
