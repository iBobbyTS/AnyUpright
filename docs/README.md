...
- Stretch object-space conversion: `AnyUprightGeometry.stretchObjectPoints`, `innerStretchObjectPoints`, `sourceCornerPercentOffset`, and `cornerPixelOffset` own the Motion/FxPlug handle coordinate semantics so corner names, X direction, and Y direction stay testable outside the host app. `Inner Stretch` stores the four handles as hidden percent offsets during OSC drags; `Outer Stretch` exposes the same offset parameters in the inspector.
