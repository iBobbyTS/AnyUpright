...
- Inner Stretch and Outer Stretch both expose `Set/Unset Key Frame`. When none of the eight corner X/Y channels has a keyframe at the host's current timeline time, one click snapshots their current values and keys all eight channels without changing those values. When any channel is already keyed at that exact time, one click removes the keyframes that exist there. Each operation is grouped into one host undo action when the undo API is available. A successful Set shows `已设置关键帧` for three seconds in the shared OSC text overlay. A successful Unset shows `已删除关键帧` plus the reminder that dragging already-keyframed clips creates a keyframe at the current frame. Both filters use the same generation-guarded transient-status controller, so repeated actions restart one shared three-second expiration path instead of scheduling competing clears. These notices are OSC-only and never enter the rendered image.
...
- Plugin defaults: Horizon, Inner Stretch, and Upright share one AppKit/FxRemoteWindow wrapper, but each opens its own editor and persists a separate strongly typed plist. The files are `~/Library/Application Support/AnyUpright/Defaults/Horizon.plist`, `InnerStretch.plist`, and `Upright.plist`. Horizon stores `Fill Frame`; Inner Stretch stores `Ratio`; Upright stores `Direction`, `Mode`, and `Auto Crop`. Each window session snapshots factory defaults and the last saved settings in memory. Restore only changes the current form to factory values, while Save is the sole operation that writes the current form to its plist; each button is enabled only when its corresponding comparison differs. Saved values are read only while registering parameters for a new filter instance. Existing instances remain fully owned and persisted by their FxPlug parameters, and render, OSC, and analysis paths never read these plist files. Outer Stretch has no defaults editor or plist.
