...
- `Mode` chooses only how reference lines are selected: `Manual` uses enabled manual guides, `Semi Auto` uses user-selected detected candidates, and `Full Auto` uses the highest-scoring detected candidates. `Analyze` is a no-op in manual mode and starts candidate-line analysis in semi/full-auto modes.
...
- Candidate detection first tries the local ScaleLSD Core ML model and Swift decode/ranking path, then falls back to the shared Sobel/Hough detector when the model resource is missing or inference fails.
...
- Automatic scoring quality. ScaleLSD ranks axis-filtered lines by normalized wireframe support score; the auto modes do not mix in candidates from the traditional detector.
...
- ScaleLSD is the primary production Upright candidate detector for Semi Auto and Full Auto. `AnyUprightScaleLSDDetector.swift` uses the shared Apple Silicon Metal resampler to produce its normalized fixed-512 grayscale input, `AnyUprightScaleLSDPreprocessor.swift` converts decoded lines into filtered candidates, `AnyUprightScaleLSDCoreML.swift` owns a model-specific facade over an independent `.fixed512` lifecycle cache, and `AnyUprightScaleLSDPostprocessor.swift` reproduces official junction/HAFM/wireframe decoding. Plugin registration asynchronously prewarms the fixed model, and Analyze uses the same configurable 15/30/60-second lifecycle policy as GeoCalib without sharing sessions or expiry state. The postprocessor uses a threshold-bounded uniform spatial grid for nearest-junction lookup; its retained linear implementation is a deterministic test oracle, not the production path. The ignored local FP16 model resource must exist at `AnyUpright/Plugin/ScaleLSDCoreML/scalelsd_neural_forward.mlmodelc`; Xcode packages it in the XPC service. If ScaleLSD cannot complete, later analysis callbacks may retry and cleanup preserves existing candidates; auto modes do not fall back to CPU Hough. Before score ranking, ScaleLSD restores the source aspect ratio and rejects lines more than 30 degrees from both the horizontal and vertical axes. The production score-only path intentionally does not call the experimental VP/proposal rejector: Semi Auto keeps the top 10 candidates per requested orientation, while Full Auto selects the top two per requested orientation because the perspective solver requires a line pair. Switching from Semi Auto to Full Auto recomputes both the displayed selection and runtime correction from candidate scores, regardless of Edit Mode, so persisted manual `selected` flags and Semi Auto correction parameters cannot leak into Full Auto. In Full direction this means up to 10V+10H for Semi Auto and 2V+2H for Full Auto. The offline proposal experiment remains in `AnyUprightUprightProposal.swift` and is not part of production writeback.
