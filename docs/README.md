...
- Click `Detect Edge and Corner`; the detected phone-screen quadrilateral should be written into the hidden corner parameters, `Edit Mode` should be enabled, and the dimmed/undimmed region plus OSC handles should mark the detected area.
- In Final Cut Pro, verify the `Quad` effect inspector shows `Edit Mode` and a `Detect Edge and Corner` button after restarting FCP. If only `Edit Mode` appears, the Motion template is missing the published custom-parameter target.
