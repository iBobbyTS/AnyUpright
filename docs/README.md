...
- `Detect Source Quad` is exposed as a custom FxPlug inspector button, backed by a non-animatable custom parameter with `kFxParameterFlag_CUSTOM_UI`. The custom parameter uses an empty `NSData` value and the Swift plug-in keeps strong references to every AppKit `NSButton` view returned from `createView(forParameterID:)` so host-side custom UI rebuilds cannot release a Swift view prematurely. Clicking it starts FxAnalysis near the current parameter time, runs Vision rectangle detection, writes the best detected rectangle into the existing hidden Source Quad corner offsets, and enables `Edit Mode` so the detected area is marked by the existing dimming and OSC handle layers. Final Cut Pro only shows this control when the Motion template publish settings include the custom parameter target (`Detect Source Quad`, channel `./216` in the current `Quad.moef`); if FCP shows only `Edit Mode`, publish/save the parameter in Motion or add the matching publish target, then restart FCP. Motion Studio 6.2 and Final Cut Pro 12.2 were verified to show the published control as a momentary `Detect` button after reopening the saved `Quad.moef` template.
...
- Click `Detect Source Quad`; the detected phone-screen quadrilateral should be written into the hidden corner parameters, `Edit Mode` should be enabled, and the dimmed/undimmed region plus OSC handles should mark the detected area.
- In Final Cut Pro, verify the `Quad` effect inspector shows both `Edit Mode` and `Detect Source Quad` after restarting FCP. If only `Edit Mode` appears, the Motion template is missing the published custom-parameter target.
