...
- Full-auto mode analyzes matching candidate lines, selects up to two highest-scoring candidates per included orientation, and displays only those selected lines while `Edit Mode` is on. The selected lines enter the same matrix solver used by Manual and Semi Auto.
- Candidate detection uses the local ScaleLSD Core ML model and Swift decode path. Semi Auto and single-direction Full Auto use the score-only selector. Full-direction Full Auto additionally runs the V2 VP/homography candidate ranker described below.
...
- Automatic scoring quality. Semi Auto and single-direction Full Auto rank axis-filtered lines by normalized ScaleLSD wireframe support score. Full-direction Full Auto uses the frozen V2 pair ranker and risk model.
...
- Full-direction Full Auto routes the raw fixed-512 lines through `AnyUprightUprightV2CandidateGenerator.swift`, which reproduces the frozen V2 line deduplication, VP clustering, GeoCalib soft prior, discrete homography generation, crop/Jacobian scoring, and supporter features. `AnyUprightUprightV2TreeModel.swift` evaluates the bundled LightGBM pair and risk models under `Plugin/UprightV2`; a risk-gated result conservatively produces no selected guides, which leaves the common solver at identity. An unavailable GeoCalib prior falls back to the V2 no-prior branch; an unavailable V2 model or selector failure falls back to the prior score-only Full Auto behavior. A selected VP family is reduced to two spatially separated representative lines per orientation and then passed to the same guided matrix solver used by Manual and Semi Auto. V2 therefore changes only line selection, not transform solving, OSC, persistence, or rendering.
- Semi Auto keeps the top 10 axis-filtered candidates per requested orientation. Vertical-only and horizontal-only Full Auto keep the top two score-ranked candidates. Switching from Semi Auto to Full Auto recomputes both displayed selection and runtime correction regardless of Edit Mode, so persisted manual `selected` flags and Semi Auto correction parameters cannot leak into Full Auto. The older offline proposal implementation in `AnyUprightUprightProposal.swift` is not part of production writeback.
