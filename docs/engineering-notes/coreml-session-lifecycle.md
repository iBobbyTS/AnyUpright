...
FxAnalysis may deliver several frames for the Horizon/Upright two-sample probe range. Each effect atomically claims only the first frame that can enter its analysis path, so one user action produces one lifecycle analysis event, including when the model result is rejected. A callback whose image cannot be prepared releases the claim for a later callback; this is frame availability handling, not a model retry policy.
