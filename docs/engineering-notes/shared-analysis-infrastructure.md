...
- `AUFxAnalysisTransaction<Request, Result>` for Horizon, Upright, and Inner Stretch request state.
...
`AUMonotonicClock` is the single source for analysis and Core ML lifecycle durations. `AUAnalysisLogger` checks a marker file and serializes append operations per process-level logger instance. Horizon, Upright/ScaleLSD, and Inner Stretch retain their existing marker and output paths; Upright and ScaleLSD intentionally share one logger and write lock.
