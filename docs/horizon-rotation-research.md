...
- Host analysis now uses the shared Apple Silicon Metal resampler through `AUGeoCalibDirectImagePreprocessor`. The resampler delegates high-quality resize to `MPSImageLanczosScale`, then packs the selected fixed-shape RGB NCHW tensor with aspect-fill center crop. The previous custom Metal Gaussian+bilinear kernel and long-edge-1920 Core Image/CPU compatibility fallback are no longer in the production path.
...
- FxPlug analysis downscale implementation options were checked against the former `FxImageTile.ioSurface` -> `CIImage` path and Apple framework availability. Core Image offers explicit Lanczos and bicubic filters; Accelerate/vImage provides CPU high-quality resampling; Metal Performance Shaders exposes bilinear, Lanczos, and Gaussian operations. The historical GT experiment above did not favor PIL Lanczos over GeoCalib-style bilinear+antialias, so the later architectural decision to standardize Horizon and Upright on MPS Lanczos must be treated as a new preprocessing variant rather than assumed numerical parity. Re-run the GeoCalib rotation quality set before using the earlier benchmark as evidence for the shared production resampler.
