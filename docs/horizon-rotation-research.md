...
- The plugin Core ML path now uses an XPC-process shared cache rather than per-effect-instance model ownership. Effect creation prewarms the common landscape model and starts a 15-second unload timer; analysis starts extend unload to 30 seconds, and two analyses inside that 30-second window extend it to 60 seconds. The portrait-shape model remains lazy and loads only when a portrait preprocessed input needs `[1, 3, 416, 320]`.
