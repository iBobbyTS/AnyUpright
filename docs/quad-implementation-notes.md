...
- Automatic Source Quad detection is a proposal/overlay path, not a writeback path, render mode, or drag state. It fills fixed hidden edge/corner primitive slots, enables `Edit Mode`, and draws detected edges as green lines plus detected corners as green crosses when their normalized scores meet `Score Threshold`. False positives are ignored by raising the threshold or corrected with the same manual handles.
