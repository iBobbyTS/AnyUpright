...
- Outer Stretch converts its four output corners into host CANVAS points before drawing. After conversion, Inner and Outer use the same `renderStretchOSC` path for active-part resolution, edge highlighting, face highlighting, circular handles, and `.canvasFramePixels` rendering. The zero-offset Outer handles therefore stay on the source object's visible image corners while preserving the same yellow drag feedback as Inner Stretch.
- Outer Stretch render fragments are clipped to the destination quadrilateral in physical output coordinates before evaluating the inverse homography. The quadrilateral is generated in the stable correction output size, scaled into the current output size, and then crossed through the physical-output Y boundary; current render dimensions must not reinterpret stable-frame pixel offsets. This output-space mask is intentionally Outer-only: it removes projective-horizon source-edge streaks under strong perspective without changing Inner Stretch Fit extrapolation or the full-frame Horizon and Upright paths.
...
- Inner Stretch owns a `Ratio` render policy independent of OSC storage. `None` maps the full output to the selected quadrilateral. `Fit` and `Fill` derive a target aspect ratio from the average opposing edge lengths and map a centered target rectangle to the same selection. Fit applies that same projective mapping across the full canvas, including outside the fitted rectangle; Fill lets the target rectangle extend beyond the output and crops it. The four stored corners are never rewritten by this policy.
