...
static float2 finalDisplayOutputCoordinate(float2 outputCoordinate, constant State& state) {
    return float2(outputCoordinate.x, state.outputSize.y - outputCoordinate.y);
}
static float2 inputTextureUV(float2 sourcePixel, constant State& state) {
    float2 texturePixel = float2(sourcePixel.x, state.inputSize.y - sourcePixel.y);
...
fragment float4 fmain(Raster in [[stage_in]], texture2d<half> input [[texture(0)]], constant State& state [[buffer(0)]]) {
    ...
    float2 outputCoordinate = clamp(finalDisplayOutputCoordinate(in.outputCoordinate, state), float2(0.0), state.outputSize);
...
var state = ShaderState(
    outputToSource: toFloat3x3(renderMatrix),
