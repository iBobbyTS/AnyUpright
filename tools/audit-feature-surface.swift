...
struct AuditFeatureSurface {
    ...
    private static func auditPluginDefaults(_ defaults: String, ui: String, warp: String) throws {
        ...
        try require(ui, "FxRemoteWindowAPI_v2", "Defaults window uses the FxPlug remote-window API")
        try require(ui, "remoteWindowAPI.remoteWindow(of: Self.contentSize)", "Defaults window requests a stable fixed content size")
        try require(ui, "rootView.frame = NSRect(origin: .zero, size: parentView.bounds.size)", "Defaults root view adopts the remote parent bounds")
        try require(ui, "rootView.autoresizingMask = [.width, .height]", "Defaults root view follows host resizing without cross-view constraints")
        ...
        try require(warp, "func presentPluginDefaults(editor: AUPluginDefaultsEditor)", "Filters open defaults through the shared wrapper")
    }
    ...
    private static func auditStretch(_ effects: String, geometry: String, overlay: String, metal: String) throws {
        ...
        try require(effects, "AUPluginDefaults.innerStretch.load().ratio", "Inner Stretch reads only its defaults store")
        ...
        try require(effects, "if showsSourceEditMode", "Only Inner Stretch registers the defaults button")
        try require(effects, "AUInnerStretchDefaultsEditor()", "Inner Stretch opens its independent defaults editor")
        try reject(effects, "AUPluginDefaults.outerStretch", "Outer Stretch has no defaults store")
